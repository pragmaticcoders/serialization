...
class SkipTestIfUnavialable(object):
    def setUp(self):
        if None in (sexp, jelly):
            raise unittest.SkipTest(skip_msg)


class SExpConvertersTest(SkipTestIfUnavialable,
                         common_serialization.ConverterTest):

    def setUp(self):
        super(SExpConvertersTest, self).setUp()
    ...
    def testJellyUnjelly(self):
        ...
        self.checkSymmetry(jelly.jelly, jelly.unjelly,
                           capabilities=caps,
                           strict=False)  # because `_Dereference`
        # instead real obj, it's jelly issue
...
class TestInstanceCreation(SkipTestIfUnavialable,
                           common.TestCase):
